...
class ResNet18MonoKernel(tf.keras.Model):
    def __init__(self, layer_params, multi_task, num_act, num_user):
        super(ResNet18MonoKernel, self).__init__()
...
def make_basic_block_layer(filter_num, blocks, name, kernel, stride=1, downsample=False):
    ...
    for _ in range(1, blocks):
        res_block.add(BasicBlock(filter_num, kernel=kernel, stride=1, downsample=downsample))

    return res_block


def resnet18MonoKernel(multi_task, num_act, num_user):
    return ResNet18MonoKernel(layer_params=[2, 2, 2, 2], multi_task=multi_task, num_act=num_act, num_user=num_user)
