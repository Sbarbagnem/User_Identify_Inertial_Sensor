...
def weight_variable_with_scope(shape, scope):
    with tf.compat.v1.variable_scope( scope ):
        w = tf.compat.v1.get_variable("weights", shape, initializer=tf.compat.v1.random_normal_initializer(stddev=0.1))
    return w
...
def bias_variable(shape):
    return tf.compat.v1.get_variable("bias", shape, initializer=tf.compat.v1.constant_initializer(0.0))
...
def conv_unit( input, w, is_training ):
    ...
    conv    = tf.nn.conv2d( input=input, filters=w, strides=[1, 1, 1, 1], padding='VALID' )
    output  = tf.compat.v1.layers.batch_normalization( conv, training=is_training )
...
def fc_unit( input, filter_shape, is_training, scope ):
    ...
    with tf.compat.v1.variable_scope( scope ):
        ...
        output  = tf.compat.v1.layers.batch_normalization( mul, training=is_training )
    return activation( output )

def pool_unit( input, ksize, stride, padding='VALID' ):
    return tf.nn.max_pool2d( input, ksize, stride, padding )
...
def lstm_unit( input, rnn_size, keep_prob, scope ):
    with tf.compat.v1.variable_scope( scope ):
        rnn_cell    = tf.compat.v1.nn.rnn_cell.BasicLSTMCell( rnn_size, forget_bias=1.0, state_is_tuple=True )
        rnn_cell    = tf.compat.v1.nn.rnn_cell.DropoutWrapper( cell=rnn_cell , input_keep_prob=1.0, output_keep_prob=keep_prob )
        output, _   = tf.compat.v1.nn.dynamic_rnn( rnn_cell, inputs=input, dtype=tf.float32, time_major=False )
    return output

def gru_unit( input, rnn_size, keep_prob, scope ):
    with tf.compat.v1.variable_scope( scope ):
        rnn_cell    = tf.compat.v1.nn.rnn_cell.GRUCell( rnn_size )
        rnn_cell    = tf.compat.v1.nn.rnn_cell.DropoutWrapper( cell=rnn_cell , input_keep_prob=1.0, output_keep_prob=keep_prob )
        output, _   = tf.compat.v1.nn.dynamic_rnn( rnn_cell, inputs=input, dtype=tf.float32, time_major=False )
    return output
'''
class MTLMA_pretrain( object ):
    ...
    def __call__( self, inputs, a_labels, u_labels, act_num, user_num, win_len, dname, fold, is_training = True, drop_keep_prob = 0.9 ):
        ...
        with tf.compat.v1.variable_scope( 'act_network' ):
            # weights of CNN
        ...
        with tf.compat.v1.variable_scope( 'user_network' ):
            # weights of CNN
        ...
        with tf.compat.v1.variable_scope( 'act_network' ):
            ...
            A_ATT       = fc_unit( A_ATT,   [A_ATT.get_shape()[1],   128],   is_training, scope='att1' )
            A_ATT       = fc_unit( A_ATT,   [128,   A_net.get_shape()[1]],   is_training, scope='att2' ) # time axis
        ...
        with tf.compat.v1.variable_scope( 'user_network' ):
            ...
            U_ATT       = fc_unit( U_ATT,   [U_ATT.get_shape()[1],   128],    is_training, scope='att1' )
            U_ATT       = fc_unit( U_ATT,   [128,   U_net.get_shape()[1]],    is_training, scope='att2' )
            U_ATT       = tf.expand_dims( tf.nn.softmax( U_ATT ), 2 )

        with tf.compat.v1.variable_scope( 'act_network' ):
            # output layer
            A_net       = tf.reduce_sum( input_tensor=tf.multiply( A_net, A_ATT ), axis=1 )
            A_net       = fc_unit_without_activiation( A_net, [A_net.get_shape()[1], act_num], is_training, scope='otpt' )       
            
        with tf.compat.v1.variable_scope( 'user_network' ):
            # output layer
            U_net       = tf.reduce_sum( input_tensor=tf.multiply( U_net, U_ATT ), axis=1 )
            U_net       = fc_unit_without_activiation( U_net, [U_net.get_shape()[1], user_num], is_training, scope='otpt' )
        ...
        U_cross_entropy = tf.reduce_mean( input_tensor=tf.nn.softmax_cross_entropy_with_logits( labels = u_labels , logits = U_net ) )
    ...
    def get_act_step_vars( self ):
        return  tf.compat.v1.get_collection( tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='act_network' )

    def get_user_step_vars( self ):
        return  tf.compat.v1.get_collection( tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='user_network' )
...
class MTLMA_train( object ):
    ...
    def __call__( self, inputs, a_labels, u_labels, act_num, user_num, win_len, dname, fold, is_training = True, drop_keep_prob = 0.9 ):
        ...
        A_cross_entropy = tf.reduce_mean( input_tensor=tf.nn.softmax_cross_entropy_with_logits( labels = a_labels , logits = A_net ) )
        U_cross_entropy = tf.reduce_mean( input_tensor=tf.nn.softmax_cross_entropy_with_logits( labels = u_labels , logits = U_net ) )
        penality 	    = sum( tf.nn.l2_loss(tf_var) for tf_var in tf.compat.v1.trainable_variables() )
