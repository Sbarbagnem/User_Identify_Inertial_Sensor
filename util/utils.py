...
def detectGaitCycle(data, plot_peak=False, plot_auto_corr_coeff=False, use_2_step=False):
    ...
    if plot_peak:
        ...
        plt.style.use('seaborn-darkgrid')
        plt.plot(np.arange(data.shape[0]), data, 'b-')
        plt.vlines(peaks, ymin=min(data)*0.95, ymax=max(data)*0.95, color='r', ls='--')
        plt.tight_layout()
        plt.show()
...
def split_data_train_val_test_gait(data, label_user, label_sequences, id_window, train_gait=8, val_test=0.5, gait_2_cycles=False, method='cycle_based', plot=False):
    ...
    if method == 'cycle_based':
        ...
        for cycles, labels, ID in zip(data_for_user, label_for_user, id_for_user):
            ...
            # validation
            val_data.append(cycles[train_percentage:train_percentage + val_percentage])
            val_label.extend(labels[train_percentage:train_percentage + val_percentage])
            val_id.extend(ID[train_percentage:train_percentage + val_percentage])

            # test
            test_data.append(cycles[train_percentage + val_percentage:])
            test_label.extend(labels[train_percentage + val_percentage:])
            test_id.extend(ID[train_percentage + val_percentage:])
...
def find_peaks(peaks, data, gcLen, use_2_step=False):
    ...
    if not use_2_step:
        ...
        while i < len(peaks):
            if peaks[i] - peaks[i-1] > 1.1*gcLen:
                ...
                try:
                    most_close_before = list(filter(lambda x: x <= temp + 0.20*gcLen and x >= temp - 0.20*gcLen, most_close))
    ...
    if not use_2_step:
        for i,peak in enumerate(peaks):
            idx  = np.where(data[peak-int(0.3*gcLen):peak+int(0.3*gcLen)] < 1.3*data[peak])
            idx = idx[0] + peak-int(0.3*gcLen)
            if len(idx) > 0:
...
def find_peaks_steps(data, step_equilibrium, step_threshold):
    ...
    while True:
        ...
        if start == 0:
            idx_step = min(list(set(np.where(data[start:] > 0.9*step_threshold)[0]) & set(maxima)))
    ...
    _mean =  int(np.sum([peaks_steps[i] - peaks_steps[i-1] for i in range(1,len(peaks_steps))]) / (len(peaks_steps)-1))

    # refine peaks based on value in neighboor
    for i in range(0,len(peaks_steps)):
        idx_r = np.where(data[peaks_steps[i]:peaks_steps[i]+int(0.3*_mean)]>data[peaks_steps[i]])[0]
        if idx_r.shape[0] > 0:
            max_neigh = np.argmax(data[peaks_steps[i]:peaks_steps[i]+int(0.3*_mean)])
    ...
    for i in range(1,len(peaks_steps)):
        if peaks_steps[i] - peaks_steps[i-1] < 0.7*_mean:
            ...
            temp = int((peaks_steps[i] - peaks_steps[i-1])/2) + peaks_steps[i-1]
            most_close = sorted(np.arange(peaks_steps[i-1], peaks_steps[i]), key=lambda x:abs(x-temp))           
            most_close_before = list(filter(lambda x: x <= temp + 0.20*_mean and x >= temp - 0.20*_mean, most_close))
            if most_close_before != []:
